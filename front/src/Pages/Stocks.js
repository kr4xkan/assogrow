...
const StocksPage = () => {
	...
	const create = (e) => {
		...
		axios.post(config.api_url + '/stocks/add', data, {
			headers: {
				'Authorization': 'Bearer ' + token
			}
		}).then((res) => refresh())
	...
	return (
		<div className="container">
			<form className="formdos formstock" onSubmit={create}>
				<AutoComplete data={partenaires} value={partenaire} onChange={(e) => setpartner(e.target.value)} />
				<div className="input">
				...
				<div className="input">
					...
					<input type="text" name="quantite" value={quantite} onChange={(e) => setquantite(e.target.value)} />
				</div>
